...
public class EventScope<T> {
     ...
     * to get garbage collected, so make sure
     * to store the receiver elsewhere, for example you could pass in a spring
     * bean.
     *
     * @author MicWin
     */
     ...
     * @return this to enable chaining.
     */
    public synchronized EventScope<T> register (final Class<? extends T> eventClass , final Object receiver , final Method handler) {
     ...
     * @return
     */
    private Method detectReceiverMethod (final Object receiver , final Class<? extends T> eventClass , final Pattern namePattern) {
        ...
        if (results.size () > 1) {
            throw new IllegalArgumentException (
                    "receiver '" + receiver + "' has more than one accessible method with a single parameter of type '" + eventClass
                    .getName () + "' " + (namePattern != null ? " and name pattern '" + namePattern.pattern () + "' " : "") + ": " + results
                    .toString ()) ;
        }

        // no hit _> exception
        throw new IllegalArgumentException (
                "receiver '" + receiver + "' does not have an accessible method with a single parameter of type '" + eventClass
                .getName () + "' " + (namePattern != null ? " and name pattern '" + namePattern.pattern () + "' " : "")) ;
    ...
    public synchronized boolean unregisterListener (final Object pHandler) {
        ...
        for (final List<ReceiverDescriptor> listenerList : this.receiverMap.values ()) {
            ...
            for (int index = 0 ; index < listenerList.size () ; index++) {
                ...
                final boolean isNull = receiverDescriptor.receiverReference == null || receiverDescriptor.receiverReference
                        .get () == null ;
